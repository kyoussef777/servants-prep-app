...
export default function CompleteRegistrationPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/registration/completion')
      .then(async (response) => {
  ...
  const uploadApprovalForm = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('/api/registration/completion/upload', {
        method: 'POST',
        body: formData,
      })
      const data = await response.json()
  ...
  const saveMentorInformation = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || 'Unable to save details')
